    ...
    def __init__(self):
        ...
        try:
            ...
            self.connection = sqlite3.connect(
                DEV_DB_PATH, check_same_thread=False)

            # table meta data
            column_meta_data = ""
            for key, value in get_our_world_in_data_attributes.items():
                column_meta_data += f"{key} {value['type']}, "
    ...
    def get_df(self, columns, table_name, where_clause=None):
        if where_clause:
            sql_query = pd.read_sql_query(
                f"SELECT {columns} FROM {table_name} WHERE {where_clause}", self.connection)
        else:
            sql_query = pd.read_sql_query(
                f"SELECT {columns} from {table_name}", self.connection)
